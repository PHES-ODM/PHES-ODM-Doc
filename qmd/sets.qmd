...
Sets are lists of categories and other parts. For example, "Collection category set" (`collectSet`) ([collectSet](https://docs.phes-odm.org/sets.html#collectSet)) contains a list of the different collection types including: `mooreSw` (Moore swab), `grb` (Grab sample), `flowPr` (Flow proportional sample), etc.
...
for (category_set_ID in unique_category_set_IDs) {
  ...
  category_input[["partInstr"]] <-
    category_info[[parts$part_instruction$name]]
